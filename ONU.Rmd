...
```{r, echo = F, out.width = '900', out.height = '600'}
...
leaflet() %>%
  ...
  addCircleMarkers(data = onu_calle, fill=T, fillColor="white", color="black", label=~paste(Artnamn) ,fillOpacity=1, weight=2,
  ...
  addArrowhead(data = onu_lines, color="white", label=~paste(Artnamn), 
               popup=~paste(sep="<br/>",as.character(Artnamn),
                            as.character(Startdatum), 
                            as.character(Aktivitet),
               ...
               weight=3) %>%
  addLayersControl(overlayGroups = "Lokaler", 
