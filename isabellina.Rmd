...
gbif_data_total <- read.delim("C:/projekt/data/0023916-231120084113126_oenanthe_isabellina.csv")
...
ggplot(data = world) +
    geom_sf() +
    coord_sf(crs=ortho, xlim=c(-5200000,5200000), ylim=c(-5000000, 3000000))
...
for(i in 1:365){
  plot1 <- ggplot(data = world) +
    ...
    #color="green", size=2)+
    ggtitle(paste0("Isabellastenskvätta ", format(as.Date(as.character(i), "%j"),"%d %b")))+
...
list.files(path=paste0(getwd(),"/out/"), pattern = '*.png', full.names = TRUE) %>% 
  ...
  image_write("gifs/oenantheisabellina.gif")
